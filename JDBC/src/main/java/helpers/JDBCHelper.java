...
public class JDBCHelper {
    ...
    public static void testDB(Connection connection) throws SQLException {
        printResult(connection, SQL_QUERY_EMPLOYEES_ORDER_BY_SALARY, LIST_OF_EMPLOYEES_BY_SALARY_MESSAGE);
        printResult(connection, SQL_QUERY_EMPLOYEES_ORDER_BY_NAME, LIST_OF_EMPLOYEES_BY_NAME_MESSAGE);
        String employeeName = "boris";
        printResult(connection, String.format(SQL_QUERY_LIST_OF_EMPLOYEES_SALARY_IS_BIGGER, employeeName), String.format(LIST_OF_EMPLOYEES_SALARY_IS_BIGGER_MESSAGE, employeeName));
        int employeeId = 5;
        printResult(connection, String.format(SQL_QUERY_GET_SALARY, employeeId), EMPLOYEE_OR_MANAGER_MESSAGE + "\nManager");
        employeeId = 6;
        printResult(connection, String.format(SQL_QUERY_GET_SALARY, employeeId), EMPLOYEE_OR_MANAGER_MESSAGE + "\nEmployee");
    }

    public static void printResult(Connection connection, String query, String message) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(query);
